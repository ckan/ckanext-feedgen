...
from ckan.lib.helpers import _url_with_params  # pyright: ignore[reportPrivateUsage]
...
def _package_search(data_dict: dict[str, Any]) -> tuple[int, list[dict[str, Any]]]:
    ...
    * unless overridden, sets a default item limit
    """
    context: Context = {"user": tk.current_user.name, "auth_user_obj": tk.current_user}
    ...
    if "rows" not in data_dict or not data_dict["rows"]:
        data_dict["rows"] = tk.config.get("ckan.feeds.limit")
...
def _enclosure(pkg: dict[str, Any]) -> Enclosure:
    url = tk.url_for("api.action", logic_function="package_show", id=pkg["name"], ver=3, _external=True)
    enc = Enclosure(url)
...
class CKANFeed(FeedGenerator):
    ...
    def writeString(self, encoding: str) -> str:  # noqa: N802
...
def output_feed(
    ...
    author_name = tk.config.get("ckan.feeds.author_name").strip() or tk.config["ckan.site_id"]
    ...
    for plugin in p.PluginImplementations(IFeed):
    ...
    for pkg in results:
        ...
        feed.add_item(
            title=pkg.get("title", ""),
            link=tk.url_for("api.action", logic_function="package_show", id=pkg["id"], ver=3, _external=True),
            description=remove_control_characters(pkg.get("notes", "")),
            updated=tk.h.date_str_to_datetime(pkg.get("metadata_modified", "")),
            published=tk.h.date_str_to_datetime(pkg.get("metadata_created", "")),
            unique_id=_create_atom_id("/dataset/{}".format(pkg["id"])),
...
def organization(id: str) -> Response:
    try:
        context: Context = {"user": tk.current_user.name, "auth_user_obj": tk.current_user}
        group_dict = logic.get_action("organization_show")(context, {"id": id})
    except logic.NotFound:
...
def tag(id: str) -> Response:
    ...
    navigation_urls = _navigation_urls(
        params, item_count=item_count, limit=data_dict["rows"], controller="feeds", action="tag", id=id
    )
    ...
    title = f'{site_title} - Tag: "{id}"'
    desc = f'Recently created or updated datasets on {site_title} by tag: "{id}"'
    guid = _create_atom_id(f"/feeds/tag/{id}.atom")
...
def group_or_organization(obj_dict: dict[str, Any], is_org: bool) -> Response:
    ...
    feed_url = _feed_url(params, controller="feeds", action=group_type, id=obj_dict["name"])
    # site_title = SITE_TITLE
    if is_org:
        guid = _create_atom_id("feeds/organization/{}.atom".format(obj_dict["name"]))
        alternate_url = _alternate_url(params, organization=obj_dict["name"])
        desc = 'Recently created or updated datasets on {} by organization: "{}"'.format(site_title, obj_dict["title"])
        title = '{} - Organization: "{}"'.format(site_title, obj_dict["title"])

    else:
        guid = _create_atom_id("feeds/group/{}.atom".format(obj_dict["name"]))
        alternate_url = _alternate_url(params, groups=obj_dict["name"])
        desc = 'Recently created or updated datasets on {} by group: "{}"'.format(site_title, obj_dict["title"])
        title = '{} - Group: "{}"'.format(site_title, obj_dict["title"])
...
def _parse_url_params() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    limit = tk.config.get("ckan.feeds.limit")
    data_dict = {"start": (page - 1) * limit, "rows": limit}

    # Filter ignored query parameters
    valid_params = ["page"]
    params = {p: tk.request.args.get(p) for p in valid_params if p in tk.request.args}
...
def general() -> Response:
    ...
    navigation_urls = _navigation_urls(
        params, item_count=item_count, limit=data_dict["rows"], controller="feeds", action="general"
    )
    ...
    site_title = tk.config.get("ckan.site_title")
    desc = f"Recently created or updated datasets on {site_title}"
...
def custom() -> Response:
    ...
    data_dict: dict[str, Any] = {
        ...
        "start": (page - 1) * limit,
        "rows": limit,
        "sort": tk.request.args.get("sort", None),
    }

    item_count, results = _package_search(data_dict)

    navigation_urls = _navigation_urls(
        tk.request.args, item_count=item_count, limit=data_dict["rows"], controller="feeds", action="custom"
    )

    feed_url = _feed_url(tk.request.args, controller="feeds", action="custom")

    atom_url = _url_with_params("/feeds/custom.atom", search_params.items())

    alternate_url = _alternate_url(tk.request.args)
    site_title = tk.config.get("ckan.site_title")
    return output_feed(
        results,
        feed_title=f"{site_title} - Custom query",
        feed_description=f"Recently created or updated datasets on {site_title}. Custom query: '{q}'",
        feed_link=alternate_url,
...
def _feed_url(query: dict[str, Any], controller: str, action: str, **kwargs: Any) -> str:
...
def _navigation_urls(
    query: dict[str, Any], controller: str, action: str, item_count: int, limit: int, **kwargs: Any
) -> dict[str, Any]:
    """Constructs and returns first, last, prev and next links for paging."""
    urls: dict[str, str | None] = dict.fromkeys(["previous", "next", "first", "last"])
    ...
    if page > 1:
        ...
        urls["previous"] = _feed_url(previous_query, controller, action, **kwargs)
    else:
        urls["previous"] = None
...
def _create_atom_id(resource_path: str, authority_name: str | None = None, date_string: str | None = None) -> str:
    ...
    if authority_name is None:
        authority_name = tk.config.get("ckan.feeds.authority_name")
        if not authority_name:
            site_url = tk.config["ckan.site_url"]
            authority_name = urlparse(site_url).netloc

    if not authority_name:
        log.warning("No authority_name available for feed generation.  Generated feed will be invalid.")
        authority_name = ""
    if date_string is None:
        date_string = tk.config.get("ckan.feeds.date")

    if not date_string:
        log.warning('No date_string available for feed generation.  Please set the "ckan.feeds.date" config value.')
...
feeds.add_url_rule("/organization/<string:id>.atom", methods=["GET"], view_func=organization)
